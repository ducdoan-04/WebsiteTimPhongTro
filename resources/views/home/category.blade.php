...
<div class="container-fluid" style="padding-left: 0px;padding-right: 0px;"></div>
<div class="container" style="min-height: 400px; margin-top:50px;">
	@if(count($listmotel) > 0)
	<?php $firstCategory = $listmotel[0]->category; ?>
	<h3 class="title-comm"><span class="title-holder" style="text-transform: uppercase;">KẾT QUẢ TÌM KIẾM THEO DANH MỤC
			{{ $firstCategory->name }} ({{ count($listmotel) }})</span></h3>
	@else
	<h3 class="title-comm"><span class="title-holder" style="text-transform: uppercase;">KẾT QUẢ TÌM
			KIẾM({{ count($listmotel) }})</span></h3>
	@endif
			...
			<div class="col-md-4 col-sm-6" id="category">
				<div class="room-item boder">
					...
					<div class="room-detail info">
						...
						<h4 class="title-category layout-1"><a href="phongtro/{{ $room->slug }}">{{ $room->title }}</a>
						</h4>
